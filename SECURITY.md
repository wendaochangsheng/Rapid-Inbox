...
- Change the bootstrap password immediately after the first startup. When `HOST` is set to a non-loopback address, the service rejects known default passwords and compatibility tokens, but this is not a substitute for separate high-entropy credentials and regular rotation. quickstart binds to `127.0.0.1` by default and warns about a TLS reverse proxy when explicitly bound externally. Do not leave `HOST` set to a loopback value and separately override Uvicorn's `--host` to an external address.
- quickstart first lets Python complete the SQLite schema and lightweight migrations, then starts ingestd only after initialization succeeds. If initialization fails, do not bypass the script and manually start the writer process first. During upgrades, stop old processes as well and allow only one migrator to operate on the database.
- The default `INGESTD_VERSION=latest` is a mutable pointer. SHA-256 verification alongside release assets can detect a corrupted or mismatched download, but it does not pin a version. Production deployments should specify an existing, reviewed release tag or build locally from a pinned source commit.
...
- Keep `HTTP_MAX_REQUEST_BODY_BYTES` at the minimum required by the application. It limits both Content-Length and chunked requests. Also configure `HTTP_REQUEST_BODY_TIMEOUT_SECONDS`, `HTTP_BODY_MEMORY_BUDGET_BYTES`, and `HTTP_CONCURRENCY_LIMIT`; quickstart passes the concurrency value to Uvicorn. These settings still do not replace reverse-proxy limits for connections, headers, rate, and timeouts.
- Use `HTTP_LIVE_CONNECTION_LIMIT` to constrain administration SSE and public-mailbox WebSockets within each process. Multi-process or multi-instance deployments must still set global long-lived-connection limits, handshake rates, and idle timeouts at the reverse proxy.
...
- Never commit `.env`, `storage/`, SQLite files, backups, logs, real email samples, or API Keys to Git. Default directory and file permissions are tightened to `0700/0600`, but a dedicated low-privilege account and encrypted disks/backups are still required.
