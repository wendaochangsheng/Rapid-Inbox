...
[![Status](https://img.shields.io/badge/status-alpha-orange.svg)](CHANGELOG.zh-CN.md)
...
[快速开始](#快速开始) · [Demo](#demo) · [项目边界](#项目边界) · [路线图](https://github.com/wendaochangsheng/Rapid-Inbox/issues/5) · [已实现能力](#当前已实现) · [配置](#配置) · [使用](#基本使用) · [贡献](CONTRIBUTING.zh-CN.md) · [安全](SECURITY.zh-CN.md)
...
> **简而言之：** Rapid Inbox 是一个 Alpha 阶段、仅接收入站邮件的自托管 SMTP 收件箱，面向
> 邮件测试、验证码流程和内部集成。它采用单机 SQLite 与本地文件系统架构，不是出站邮件服务器、
> 托管邮箱服务或多节点邮件平台。
...
运行时和开发依赖在 `pyproject.toml` 中固定到精确版本，`constraints-dev.txt` 保存一组经过验证的开发依赖解析结果。已有虚拟环境拉取新代码后，建议重新执行安装命令，确保入口脚本和依赖版本一致。
...
安全问题请优先查看 [SECURITY.zh-CN.md](SECURITY.zh-CN.md)。

## 贡献

欢迎提交 Issue、修复和改进。项目由 [@wendaochangsheng](https://github.com/wendaochangsheng) 维护。
开始前建议先阅读 [CONTRIBUTING.zh-CN.md](CONTRIBUTING.zh-CN.md)，里面包含开发流程、测试方式和提交 PR 的注意事项。
